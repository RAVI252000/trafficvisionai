    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 border-b border-white/[0.06] pb-6 sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-white">AI Reports & Digests</h1>
      ...
      <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Generated Reports</span>
          <div className="mt-2 text-2xl font-bold text-white">{summary?.total_reports || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-tv-primary">Daily Digests</span>
          <div className="mt-2 text-2xl font-bold text-tv-primary">{summary?.daily_count || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-emerald-400">Weekly Summary Digests</span>
          <div className="mt-2 text-2xl font-bold text-emerald-500">{summary?.weekly_count || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-amber-400">Monthly Aggregations</span>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl xl:col-span-1 h-fit">
          ...
          {isAdmin ? (
            <form onSubmit={handleCreateReport} className="flex flex-col gap-4">
              {/* Report Type */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Report Type</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/[0.08] bg-slate-900 px-3 py-1.5 text-xs text-white focus:outline-none"
                  />
                </div>
                <div>
              ...
              {/* Report Name */}
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Report Name</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <button
                type="submit"
                disabled={generating}
                className="mt-2 w-full flex items-center justify-center gap-2 rounded-xl bg-tv-primary py-2.5 text-sm font-semibold text-white shadow-lg shadow-tv-primary/20 hover:bg-blue-500 transition disabled:opacity-50"
              >
            ...
            <div className="flex flex-col items-center justify-center text-center p-6 border border-white/[0.04] bg-white/[0.01] rounded-2xl">
              <Shield className="h-8 w-8 text-slate-500 mb-2" />
              <span className="text-xs font-semibold text-slate-300">Generate Restricted</span>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl xl:col-span-2">
          <h3 className="mb-4 text-sm font-bold text-white uppercase tracking-wider flex items-center gap-1.5">
          ...
          {loading && reports.length === 0 ? (
            <div className="flex h-80 flex-col items-center justify-center gap-3">
              ...
              <span className="text-sm font-medium text-slate-400">Loading reports archive...</span>
            </div>
          ) : reports.length === 0 ? (
            <div className="flex h-80 flex-col items-center justify-center text-center">
              <AlertCircle className="h-8 w-8 text-slate-500 mb-2" />
              <h4 className="font-bold text-white text-sm">Reports Archive Empty</h4>
            ...
            <div className="flex flex-col gap-3">
              {reports.map(rep => (
                <div
                  key={rep.id}
                  className="flex flex-col justify-between p-4.5 rounded-2xl border border-white/[0.04] bg-white/[0.01] hover:bg-white/[0.02] transition sm:flex-row sm:items-center gap-4"
                >
                  <div>
                    <span className="text-[10px] font-bold text-tv-primary uppercase tracking-wider">{rep.report_type}</span>
                    <h4 className="text-sm font-bold text-white mt-1 leading-tight">{rep.name}</h4>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-white/[0.08] bg-white/[0.02] px-2.5 py-1.5 text-xs text-slate-300 hover:text-white transition"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center gap-1 rounded-lg bg-slate-500/10 border border-white/[0.08] px-2 py-1.5 text-[10px] font-semibold text-slate-300 hover:bg-white/[0.06] transition disabled:opacity-40"
                    >
                      <Download className="h-3 w-3" />
        ...
        {selectedReport && (
          <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs">
            ...
            <motion.div
              ...
              className="relative h-full w-full max-w-xl border-l border-white/[0.08] bg-[#0F172A] p-6 shadow-2xl overflow-y-auto flex flex-col justify-between"
            >
              <div>
                {/* Header */}
                <div className="mb-6 flex items-center justify-between border-b border-white/[0.06] pb-4">
                  <div>
                    ...
                    <h3 className="text-base font-bold text-white mt-1">{selectedReport.name}</h3>
                  </div>
                  <button
                ...
                <div className="flex flex-col gap-6 text-xs text-slate-300">
                  {/* Metadata */}
                  <div className="rounded-xl border border-white/[0.04] bg-white/[0.01] p-3 text-slate-400">
                    <span className="font-bold text-slate-300 block mb-1">Compilation Metadata</span>
                    Generated At: <span className="text-white font-mono">{selectedReport.summary_data.metadata.generated_at}</span>
                    <br/>
                    Applied Filters: <span className="text-white font-mono">{JSON.stringify(selectedReport.summary_data.metadata.filters)}</span>
                  </div>

                  {/* Executive Summary */}
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Executive Summary</span>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Total Vehicles</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
                          {selectedReport.summary_data.kpis.total_traffic_count.toLocaleString()}
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Avg Density</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
                          {selectedReport.summary_data.kpis.average_congestion_score}%
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Peak Hour</span>
                        <span className="text-sm font-bold text-amber-500 mt-0.5 block">
                          {selectedReport.summary_data.kpis.peak_hour}
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Travel Time</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
                          {selectedReport.summary_data.kpis.average_travel_time_minutes}m
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Accuracy Index</span>
                        <span className="text-sm font-bold text-emerald-400 mt-0.5 block">
                          {selectedReport.summary_data.kpis.prediction_accuracy_pct}%
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Monitored Roads</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
                  ...
                  {/* Hotspots */}
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Segment Hotspots</span>
                    <div className="flex flex-col gap-2 bg-slate-900/40 border border-white/[0.04] p-3.5 rounded-xl">
                      <div>
                        <span className="text-slate-400 block text-[10px]">MOST CONGESTED segments</span>
                      ...
                      <div className="mt-2.5">
                        <span className="text-slate-400 block text-[10px]">LEAST CONGESTED segments</span>
                        <span className="text-white font-bold text-sm mt-0.5 block">
                  ...
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Safety & Advisories Summary</span>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">Total Incident Alerts</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
                          {selectedReport.summary_data.alerts.total_registered}
                        </span>
                      </div>
                      <div className="rounded-xl bg-slate-900/60 p-3 border border-white/[0.04]">
                        <span className="text-slate-400 block text-[10px] uppercase">AI Decisions Generated</span>
                        <span className="text-sm font-bold text-white mt-0.5 block">
              ...
              <div className="mt-8 border-t border-white/[0.06] pt-4.5">
                <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2.5">Download File</span>
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <button
                    ...
                    disabled={downloading !== null}
                    className="flex items-center justify-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.02] text-slate-300 py-2.5 text-xs font-semibold hover:bg-white/[0.06] transition disabled:opacity-40"
                  >
