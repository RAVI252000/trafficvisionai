    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 border-b border-white/[0.06] pb-6 sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-white">AI-Driven Traffic Trend Analysis</h1>
      ...
      <div className="mb-8">
        ...
        {loading && !trends ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="h-32 rounded-2xl bg-white/[0.01] animate-pulse border border-white/[0.04]" />
            <div className="h-32 rounded-2xl bg-white/[0.01] animate-pulse border border-white/[0.04]" />
            <div className="h-32 rounded-2xl bg-white/[0.01] animate-pulse border border-white/[0.04]" />
          </div>
        ) : (
                ...
                <div key={insight.id} className={`rounded-2xl border p-4.5 flex flex-col justify-between shadow-lg ${bg}`}>
                  <div>
                    <div className="flex items-start justify-between gap-3">
                      <div className="font-bold text-sm text-slate-100 flex items-center gap-1.5 leading-tight">
                        {icon}
                        {insight.title}
                      </div>
                      {insight.impact_percentage !== null && (
                        <span className={`text-xs font-extrabold px-1.5 py-0.5 rounded bg-slate-900 border border-white/[0.08] flex items-center gap-0.5 ${insight.impact_percentage > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                  ...
                  <div className="mt-4 text-[9px] text-slate-500 font-medium">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="flex flex-col gap-6 lg:col-span-8">
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl">
            <div className="mb-6 flex flex-col justify-between gap-4 border-b border-white/[0.04] pb-4 md:flex-row md:items-center">
              <div>
                <h3 className="text-base font-bold text-white flex items-center gap-2">
                  <Activity className="h-4 w-4 text-tv-primary" />
                  Historical vs Predicted Traffic Flow Comparison
                </h3>
                <p className="text-xs text-slate-400">Comparing average baseline motor counts against forecasts.</p>
              </div>

              {/* Chart Tabs selector */}
              <div className="flex rounded-xl bg-slate-950 p-1 border border-white/[0.04]">
              ...
              <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={getActiveDataset()}>
                    ...
                    <Line type="monotone" dataKey="historical" stroke="#3b82f6" strokeWidth={2.5} name="Historical baseline" dot={{ r: 3 }} activeDot={{ r: 5 }} />
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl">
            <div className="mb-4 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="text-xs text-slate-400">Search predictions for a specific segment to overlay hourly speeds.</p>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="rounded-xl border border-white/[0.08] bg-slate-900 pl-9 pr-4 py-2 text-sm text-white focus:border-tv-primary focus:outline-none"
                />
              </div>
            </div>
            ...
            {forecastLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {/* Meta details */}
                <div className="md:col-span-1 rounded-2xl border border-white/[0.04] bg-[#1E293B]/20 p-4.5 flex flex-col justify-between">
                  <div>
                    <span className="text-xs font-bold text-tv-primary tracking-wide uppercase">Segment Metadata</span>
                    <h4 className="mt-2 text-xl font-bold text-white">{forecastData.road_name}</h4>
                    <p className="mt-1 text-xs text-slate-400">Category: {forecastData.road_type}</p>
                    
                    <div className="mt-4 flex flex-col gap-2.5 text-xs text-slate-300">
                      <div className="flex justify-between">
                        <span>Latitude:</span>
                        <span className="font-mono text-white">{forecastData.latitude.toFixed(4)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Longitude:</span>
                        <span className="font-mono text-white">{forecastData.longitude.toFixed(4)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Timeline Date:</span>
                        <span className="font-mono text-white">{forecastData.date}</span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex flex-col gap-6 lg:col-span-4">
          ...
          <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl">
            <h3 className="mb-4 flex items-center gap-1.5 text-sm font-bold text-white">
              <Activity className="h-4 w-4 text-tv-primary" />
            ...
            {loading && !trends ? (
              ...
              <div className="flex flex-col gap-3">
                {trends?.region_trends.map((item: any) => {
                  ...
                  return (
                    <div key={item.region} className="flex items-center justify-between border-b border-white/[0.04] pb-2.5">
                      <div>
                        <span className="text-xs font-bold text-slate-200 block">{item.region}</span>
                        <span className="text-[10px] text-slate-500">Congestion: {item.avg_congestion}%</span>
                      </div>
