...
export function RecommendationsPage() {
  ...
  const getStatusBadgeClass = (statusVal: string) => {
    switch (statusVal) {
      ...
      case 'Dismissed': return 'bg-slate-500/10 border-white/[0.08] text-slate-400'
    ...
    <div className="min-h-screen bg-slate-950 p-6 text-slate-100">
      ...
      <div className="mb-8 flex flex-col justify-between gap-4 border-b border-white/[0.06] pb-6 sm:flex-row sm:items-center">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-white">AI Recommendations Dashboard</h1>
        ...
        <div className="flex gap-3">
          ...
          {isAdmin ? (
            <button
              ...
              disabled={generating}
              className="flex items-center gap-2 rounded-xl bg-tv-primary px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-tv-primary/20 transition hover:bg-blue-500 hover:shadow-tv-primary/30"
            >
              <Sparkles className="h-4 w-4" />
              {generating ? 'Compiling AI Logic...' : 'Trigger AI Engine'}
            </button>
          ) : (
            <div className="flex items-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.02] px-3.5 py-2 text-xs text-slate-400">
      ...
      <div className="mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-blue-500/10 bg-blue-500/5 p-4.5 flex gap-3.5 items-start">
          ...
          <div>
            <span className="font-bold text-xs text-white block">Evening Flow Capacity</span>
            <p className="text-xs text-slate-300 mt-1">Congestion is expected to increase by 23% during evening rush hours on link corridors.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-emerald-500/10 bg-emerald-500/5 p-4.5 flex gap-3.5 items-start">
          ...
          <div>
            <span className="font-bold text-xs text-white block">Bypass Optimization</span>
            <p className="text-xs text-slate-300 mt-1">Route B alternative diversion can reduce commuter travel times by 18%.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-amber-500/10 bg-amber-500/5 p-4.5 flex gap-3.5 items-start">
          ...
          <div>
            <span className="font-bold text-xs text-white block">Adaptive Intersections</span>
            <p className="text-xs text-slate-300 mt-1">Traffic signal optimization at Junction A can reduce baseline queuing indices.</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-red-500/10 bg-red-500/5 p-4.5 flex gap-3.5 items-start">
          ...
          <div>
            <span className="font-bold text-xs text-white block">Critical Enforcement</span>
            <p className="text-xs text-slate-300 mt-1">Road X requires additional CCTV/speed monitoring due to recurrent congestion spikes.</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Total Advisory Alerts</span>
          <div className="mt-2 text-2xl font-bold text-white">{summary?.total_count || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-red-400">Critical Priority</span>
          <div className="mt-2 text-2xl font-bold text-red-500">{summary?.critical_count || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-amber-400">Pending Review</span>
          <div className="mt-2 text-2xl font-bold text-amber-500">{summary?.pending_count || 0}</div>
        </div>
        <div className="rounded-2xl border border-white/[0.05] bg-[#1E293B]/40 p-4.5">
          <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider text-emerald-400">Implemented Actions</span>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-4">
        {/* Left Column: Filters */}
        <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl xl:col-span-1 h-fit">
          ...
          <div className="flex flex-col gap-4">
            {/* Search */}
            <div>
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Keyword Search</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/[0.08] bg-slate-900 pl-9 pr-4 py-2 text-sm text-white focus:outline-none"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/[0.08] bg-slate-900 px-3.5 py-2 text-sm text-white focus:outline-none"
              />
            </div>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-[#0F172A] p-5 shadow-xl xl:col-span-3">
          {loading && recommendations.length === 0 ? (
            <div className="flex h-96 flex-col items-center justify-center gap-3">
              <RefreshCw className="h-8 w-8 animate-spin text-tv-primary" />
              <span className="text-sm font-medium text-slate-400">Loading AI Recommendations...</span>
            </div>
          ) : recommendations.length === 0 ? (
            <div className="flex h-96 flex-col items-center justify-center text-center">
              <AlertCircle className="h-10 w-10 text-slate-500 mb-3" />
              <h4 className="font-bold text-white text-base">No Recommendations Found</h4>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-white/[0.06] text-xs font-semibold text-slate-400 uppercase">
                ...
                <tbody className="divide-y divide-white/[0.04] text-slate-300">
                  {recommendations.map(rec => (
                    ...
                    >
                      ...
                      <td className="py-4.5 pr-4 font-bold text-white whitespace-nowrap">
                        {rec.affected_road}
                      </td>
                      <td className="py-4.5 pr-4 max-w-[240px] truncate font-medium text-white/90">
                        {rec.title}
                      </td>
                      ...
                      <td className="py-4.5 text-right text-xs text-slate-500 whitespace-nowrap font-mono">
        ...
        {selectedRec && (
          <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-xs">
            ...
            <motion.div
              ...
              className="relative h-full w-full max-w-lg border-l border-white/[0.08] bg-[#0F172A] p-6 shadow-2xl overflow-y-auto flex flex-col justify-between"
            >
              <div>
                {/* Header */}
                <div className="mb-6 flex items-center justify-between border-b border-white/[0.06] pb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-white mt-1 leading-tight">{selectedRec.title}</h3>
                  </div>
                  <button
                    onClick={() => setSelectedRec(null)}
                    className="rounded-lg p-1.5 text-slate-400 hover:bg-white/[0.04] hover:text-white"
                  >
                    <X className="h-5 w-5" />
                  </button>
                </div>
                ...
                <div className="flex flex-col gap-5 text-sm text-slate-300">
                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase mb-1.5">Actionable Description</span>
                    <p className="rounded-xl border border-white/[0.04] bg-white/[0.01] p-3 text-xs leading-relaxed text-slate-200">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Priority</span>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Affected Roadway</span>
                      <span className="text-white font-bold text-sm">{selectedRec.affected_road}</span>
                    </div>

                    <div>
                      <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Region Jurisdiction</span>
                      <span className="text-slate-300 text-xs">{selectedRec.region}</span>
                    </div>
                  </div>

                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Confidence Score</span>
                    <div className="flex items-center gap-3">
                      <div className="flex-1 h-2 rounded-full bg-slate-800">
                        <div className="h-full rounded-full bg-tv-primary" style={{ width: `${selectedRec.confidence_score * 100}%` }} />
                      </div>
                      <span className="text-sm font-bold text-white font-mono">{Math.round(selectedRec.confidence_score * 100)}%</span>
                    </div>
                  </div>

                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Trigger Telemetry / Reason</span>
                    <p className="text-xs leading-relaxed text-slate-400 bg-slate-900/50 p-3 rounded-xl border border-white/[0.04]">
                      {selectedRec.reason}
                    </p>
                  </div>

                  <div>
                    <span className="block text-xs font-semibold text-slate-500 uppercase mb-1">Expected Capacity Impact</span>
              ...
              <div className="mt-8 border-t border-white/[0.06] pt-4.5">
                <span className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2.5">Update Status</span>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    onClick={() => handleUpdateStatus('Accepted')}
                    disabled={updatingStatus || selectedRec.status === 'Accepted'}
                    className="flex items-center justify-center gap-1.5 rounded-xl border border-blue-500/20 bg-blue-500/10 text-blue-400 py-2.5 text-xs font-semibold hover:bg-blue-500/20 transition disabled:opacity-40"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-1.5 rounded-xl border border-white/[0.08] bg-white/[0.02] text-slate-400 py-2.5 text-xs font-semibold hover:bg-white/[0.06] transition disabled:opacity-40 col-span-2 mt-1"
                  >
